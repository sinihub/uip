...
public class InterfaceQuotaStrategy extends IQuotaComputStrategy {
    ...
    @Override
    public Object execIndiComput(QuotaTemp quotaTemp, Calendar computCal) throws EvaluationException {
        ...
        String result = null;
        // new DateTimeFunction()和 new InstantFunction()
        List<Function> functionList = logicalFunctions.getFunctions();
        for(Function function:functionList) {
            // dateTime -> function.getName()
            if (calculateExp.contains(function.getName())) {
                ...
                try {
                    ...
                    /* 特殊处理复合inst(...)函数 */
                    Integer inst = calculateExp.indexOf("inst");
                    Integer firstRight = calculateExp.indexOf(")");
                    String prefix = calculateExp.substring(0,inst);
                    String suffix = calculateExp.substring(firstRight + 1,calculateExp.length());
                    if(StringUtils.isNotBlank(prefix) && StringUtils.isNotBlank(suffix)){
                        newExpress = prefix + computExp + suffix;
                        result = evaluator.evaluate(newExpress);
                    } else {
                    ...
                    throw new EvaluationException("指标计算出现错误!");
                } catch (Exception e) {
        ...
        for (OriginalData originalData : originalDataList) {
            ...
            BigDecimal bigDecimal = new BigDecimal(itemValue.trim());
            itemValue = bigDecimal.toPlainString();
