...
public class TradeDataRepositoryImpl {
    ...
    public List<LoadometerIndicatorDto> queryForLoadometerIndicator(String queryDate){
        ...
        nativeSql.append(" SELECT ")
                ...
                .append("      t_trade T ")
                .append("   ON T.productcode like CONCAT('%',MP.sourceCode,'%')  ")
    ...
    /**
     * 根据给定的日期查询当前时间所拥有的地磅数据数量
     * zzx
     * @param queryDate
     * @return
     */
    public List<Object>  countDataBySecondTime(String queryDate){
        StringBuilder nativeSql = new StringBuilder();
        EntityManager em = emf.createEntityManager();
        nativeSql.append("SELECT COUNT(*) FROM t_trade where DATE_FORMAT(secondWeightTime,'%Y%m%d') = ?1");
        Query query = em.createNativeQuery(nativeSql.toString());
